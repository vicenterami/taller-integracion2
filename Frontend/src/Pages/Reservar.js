  ...
  const obtenerCitasDisponibles = async () => {
    try {
      const response = await axios.get(
        "http://45.236.129.38:3000/api/citas/disponibles"
      );
  ...
  const reservarCita = async (citaId) => {
    try {
      const response = await axios.post(
        "http://45.236.129.38:3000/api/citas/reservar",
        {
          citaId,
        ...
        <div className="row justify-content-center">
          {citasDisponibles.map((cita) => (
            <div key={cita._id} className="col-md-4 mb-3">
              <div className="card border-primary" style={{ padding: "10px" }}>
                ...
                <Toaster
                  richColors
                  position="top-center"
                  toastOptions={{ style: { width: "100%", height: "auto" } }}
                />
                {
                  <button
                    className="btn btn-primary"
                    onClick={() => reservarCita(cita._id)}
                  >
                    Reservar Cita
                  </button>
                }
